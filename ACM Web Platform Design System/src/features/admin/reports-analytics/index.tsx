...
export const ReportsAnalytics: React.FC = () => {
  ...
  const buildDrilldownQuery = (
    ...
    if (appliedFilters.farmId !== "all")
      params.set("farmId", appliedFilters.farmId);
    if (appliedFilters.plotId !== "all")
      params.set("plotId", appliedFilters.plotId);
    if (appliedFilters.cropId !== "all")
      params.set("cropId", appliedFilters.cropId);
    if (appliedFilters.varietyId !== "all")
      params.set("varietyId", appliedFilters.varietyId);
  ...
  const handleDrilldown = (target: "yield" | "cost" | "revenue" | "profit") => {
    ...
    const routes: Record<typeof target, string> = {
      yield: "/admin/harvest",
      cost: "/admin/expenses",
      revenue: "/admin/harvest",
      profit: "/admin/reports",
    };
    const query = buildDrilldownQuery({});
    navigate(`${routes[target]}?${query}`);
  };
  ...
  const handleRowClick = (
    ...
    const query = buildDrilldownQuery({
      farmId: rowData.farmId ?? undefined,
      plotId: rowData.plotId ?? undefined,
      cropId: rowData.cropId ?? undefined,
      varietyId: rowData.varietyId ?? undefined,
      category: rowData.category ?? undefined,
      vendorId: rowData.vendorId ?? undefined,
    });
    const routes: Record<typeof tab, string> = {
      yield: "/admin/harvest",
      cost: "/admin/expenses",
      revenue: "/admin/harvest",
      profit: "/admin/reports",
    };
    navigate(`${routes[tab]}?${query}`);
  };
