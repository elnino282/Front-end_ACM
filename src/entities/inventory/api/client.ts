...
export const inventoryApi = {
     ...
     */
    getMyWarehouses: async (type?: string): Promise<Warehouse[]> => {
    ...
    getWarehouseById: async (id: number): Promise<Warehouse> => {
        const response = await httpClient.get(`/api/v1/inventory/warehouses/${id}`);
    ...
    createWarehouse: async (data: CreateWarehouseRequest): Promise<Warehouse> => {
        const payload = CreateWarehouseRequestSchema.parse(data);
        const response = await httpClient.post('/api/v1/inventory/warehouses', payload);
        return parseApiResponse(response.data, WarehouseSchema);
    },
     ...
     */
    updateWarehouse: async (id: number, data: UpdateWarehouseRequest): Promise<Warehouse> => {
        const payload = UpdateWarehouseRequestSchema.parse(data);
        const response = await httpClient.patch(`/api/v1/inventory/warehouses/${id}`, payload);
        return parseApiResponse(response.data, WarehouseSchema);
    },
    ...
    deleteWarehouse: async (id: number): Promise<boolean> => {
        const response = await httpClient.delete(`/api/v1/inventory/warehouses/${id}`);
